...
import org.shrek.hadata.service.qimen.config.QimenToBConfig;
 ...
 */
@Service
public class Qimen100Schedule {
    private static String url = "http://qimen.api.taobao.com/router/qimen/service";
    //private static String url = "http://qimen.api.taobao.com/router/qmtest";
        ...
        qimenConfig.getApps().forEach((k, v) -> {
                    ...
                    for (int i = 0; i < tPoMasters.size(); i++) {
                        ...
                        for (int j = 0; j < details.size(); j++) {
                            ...
                            orderLine.setActualQty(Math.round(detail.getActualQty()));
                            orderLines.add(orderLine);
                        }
                        request.setOrderLines(orderLines);
                        String json = JacksonUtil.nonAlways().toJson(request);
                        System.out.println("收货回传回传报文：" + json);
                        try {
                            EntryorderConfirmResponse response = _execute(request);
                            if (response.getFlag().equals("success")) {
                                inBoundService.updateInBoundsBack(tPoMaster.getWhId(), tPoMaster.getPoNumber(),"回传成功",new Date());
                            } else{
                                inBoundService.updateInBoundsUnback(tPoMaster.getWhId(), tPoMaster.getPoNumber(),response.getMessage().substring(1,20),new Date());
                            }
...
//        if (scheduled) {
        List<TPoMaster> tPoMasters = inBoundService.queryConfirmInBoundsByWhse("682", "402", 2453);
        for (int i = 0; i < tPoMasters.size(); i++) {
            TPoMaster tPoMaster = tPoMasters.get(i);
            EntryorderConfirmRequest request = new EntryorderConfirmRequest();
            request.setCustomerId("c1542593488854");
            EntryorderConfirmRequest.EntryOrder entryOrder = new EntryorderConfirmRequest.EntryOrder();
            entryOrder.setOutBizCode(UUID.randomUUID().toString());
            entryOrder.setEntryOrderCode(tPoMaster.getPoNumber());
            entryOrder.setWarehouseCode("402");
            entryOrder.setOwnerCode("682");
...
//        }
        ...
        qimenConfig.getApps().forEach((k, v) -> {
            ...
            for (int i = 0; i < tPoMasters.size(); i++) {
                ...
                for (int j = 0; j < details.size(); j++) {
                    ...
                    //应收商品数量
                    orderLine.setPlanQty(Math.round(detail.getQty()));
                    orderLine.setActualQty(detail.getActualQty().toString());
                    orderLines.add(orderLine);
                }
                request.setOrderLines(orderLines);
                try {
    ...
    public void feedbackDeliveryOrderConfirm() {
        qimenConfig.getApps().forEach((k, v) -> {
            ...
            for (int i = 0; i < orders.size(); i++) {
                ...
                pack.setExpressCode(tOrder.getCarrierNumber() == null ? "" : tOrder.getCarrierNumber());
                ...
                for (int m = 0; m < detailList.size(); m++) {
                    ...
                    item.setQuantity(tOrderDetail.getQtyShipped().longValue());
    ...
    public void feedbackStockoutOrderConfirm() {
        qimenConfig.getApps().forEach((k, v) -> {
            List<TOrder> orders = outBoundService.queryConfirmOutBoundsByWhseAndClientAndType(v.getWarehouse(), v.getClient(), 2456);
            for (int i = 0; i < orders.size(); i++) {
                TOrder tOrder = orders.get(i);
